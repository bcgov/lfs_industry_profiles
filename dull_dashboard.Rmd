...
my_biplot <- function(pcs){
  rownames(pcs$x) <- word(rownames(pcs$x), 1)
  plt <- autoplot(pcs,
                  label=TRUE,
                  label.size=2,
                  shape=4,
...
time_series<- read_rds(here::here("out","for_pca.rds"))%>%
  mutate(biplot=map(pcs, my_biplot))

page_titles <- c("Title Page",#replace with your page titles
                 "Data table",
                 "Level Changes",
                 "Percent Changes",
                 "Biplots of Time series features",
                 "Page 6",
                 "Page 7",
                 "Page 8",
                 "Page 9",
