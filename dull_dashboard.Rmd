...
time_series_plot <- function(shared_df){ 
...
time_series<- read_rds(here::here("out","smoothed_with_mapping.rds"))%>%
  unnest(data)%>%
  pivot_longer(cols=-c(agg_level,name,high,medium,low), names_to = "date", values_to = "value")%>%
  filter(agg_level==high)

Shared_time_em <- SharedData$new(time_series%>%filter(name=="Employed"), group = "indus")
Shared_time_ft <- SharedData$new(time_series%>%filter(name=="Full-Time"), group = "indus")
Shared_time_lf <- SharedData$new(time_series%>%filter(name=="Labour Force"), group = "indus")
Shared_time_pt <- SharedData$new(time_series%>%filter(name=="Part-Time"), group = "indus")
Shared_time_un <- SharedData$new(time_series%>%filter(name=="Unemployed"), group = "indus")
Shared_time_ur <- SharedData$new(time_series%>%filter(name=="Unemployment Rate"), group = "indus")
...
```{r}
time_series_plot(Shared_time_em)
```
...
```{r}
time_series_plot(Shared_time_ft)
```

### Labour Force

```{r}
time_series_plot(Shared_time_lf)
```
...
```{r}
time_series_plot(Shared_time_pt)
```

### Unemployed

```{r}
time_series_plot(Shared_time_un)
```

### Unemployment rate

```{r}
time_series_plot(Shared_time_ur)
```
